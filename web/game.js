...
heroFace.src = 'assets/hero_face.png';
...
const saveKey = 'remontnik-save-v1';
let save = loadSave();
...
function drawPlayer(){
  ...
  const run=player.onGround?Math.sin(player.runTime*7):0;
  const squash=player.landSquash*.12;
  ctx.save();ctx.translate(x,y);ctx.scale(player.facing*(1+squash),1-squash);
  // shadow
  ctx.save();ctx.scale(player.facing,1);ctx.fillStyle='rgba(0,0,0,.22)';ctx.beginPath();ctx.ellipse(0,2,32,8,0,0,TAU);ctx.fill();ctx.restore();
  // legs and shoes
  ctx.strokeStyle='#244769';ctx.lineWidth=16;ctx.lineCap='round';ctx.beginPath();ctx.moveTo(-10,-42);ctx.lineTo(-12+run*10,-7);ctx.moveTo(10,-42);ctx.lineTo(12-run*10,-7);ctx.stroke();
  ctx.strokeStyle='#20242a';ctx.lineWidth=13;ctx.beginPath();ctx.moveTo(-13+run*10,-5);ctx.lineTo(-25+run*10,-1);ctx.moveTo(13-run*10,-5);ctx.lineTo(25-run*10,-1);ctx.stroke();
  // body black shirt
  ctx.fillStyle='#15181d';ctx.beginPath();ctx.roundRect(-28,-91,56,55,15);ctx.fill();
  ctx.fillStyle='#d4a04e';ctx.fillRect(-28,-42,56,7);
  // arm
  ctx.strokeStyle='#d9a181';ctx.lineWidth=14;ctx.lineCap='round';
  const hammering=player.hammerTimer>0 || state==='cutscene';
  let armA=run*.22;
  if(hammering){ const tt=state==='cutscene'?Math.sin(cutscene.time*8):Math.sin((.28-player.hammerTimer)*20); armA=-1.1+tt*.9; }
  ctx.save();ctx.rotate(armA);ctx.beginPath();ctx.moveTo(18,-78);ctx.lineTo(34,-47);ctx.stroke();
  if(hammering){ctx.strokeStyle='#87572f';ctx.lineWidth=7;ctx.beginPath();ctx.moveTo(34,-48);ctx.lineTo(65,-88);ctx.stroke();ctx.fillStyle='#6b737d';ctx.fillRect(48,-103,42,19);}
  ctx.restore();
  ctx.beginPath();ctx.moveTo(-18,-77);ctx.lineTo(-31,-47);ctx.stroke();
  // neck
  ctx.fillStyle='#d9a181';ctx.fillRect(-10,-103,20,18);
  // head with supplied face
  ctx.save();ctx.beginPath();ctx.ellipse(0,-124,31,39,0,0,TAU);ctx.clip();
  if(heroFace.complete && heroFace.naturalWidth){ctx.drawImage(heroFace,-38,-168,76,88);}else{ctx.fillStyle='#d7a17f';ctx.fillRect(-35,-168,70,88);}
  ctx.restore();ctx.strokeStyle='rgba(20,20,25,.7)';ctx.lineWidth=3;ctx.beginPath();ctx.ellipse(0,-124,31,39,0,0,TAU);ctx.stroke();
  ctx.restore();
}
...
function drawMenu(){
  ...
  // hero large
  const temp=player;player={x:170,y:390,w:52,h:106,facing:1,onGround:true,runTime:stateTime*.3,landSquash:0,inv:0,hammerTimer:0};drawPlayer();player=temp;
